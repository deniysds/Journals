...
@section('breadcrumbs')
    <div class="flex items-center gap-2 text-xs text-gray-500 mb-4">
        ...
        <span>/</span>
        <span class="font-semibold text-gray-800">Tambah Pengumuman</span>
    </div>
@endsection
...
<div class="max-w-4xl mx-auto space-y-6">
    <div class="kt-card p-6 bg-white rounded-2xl border border-slate-200/80 shadow-xs space-y-6">
        <div class="border-b border-slate-100 pb-4">
            <h1 class="text-xl font-bold text-slate-900">Buat Pengumuman Baru / Call for Papers</h1>
            <p class="text-xs text-slate-500 mt-1">Isi formulir di bawah ini untuk menerbitkan pengumuman atau ajakan penyerahan naskah.</p>
        </div>
        ...
        <form action="{{ route('announcements.store') }}" method="POST" enctype="multipart/form-data" class="space-y-5">
            @csrf
            ...
            <div>
                ...
                <input type="text" name="title" value="{{ old('title') }}" required placeholder="Contoh: Call for Papers: Edisi Khusus Riset Genomik Tropis 2026" class="w-full px-3.5 py-2.5 rounded-xl border border-slate-200 text-xs focus:ring-2 focus:ring-red-500 outline-hidden" />
            </div>
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                ...
                <div>
                    ...
                    <select name="type" required class="w-full px-3.5 py-2.5 rounded-xl border border-slate-200 text-xs focus:ring-2 focus:ring-red-500 outline-hidden bg-white">
                        <option value="call_for_papers" {{ old('type') === 'call_for_papers' ? 'selected' : '' }}>Call for Papers (CFP)</option>
                        <option value="announcement" {{ old('type', 'announcement') === 'announcement' ? 'selected' : '' }}>Pengumuman Redaksi / Publikasi</option>
                        <option value="event" {{ old('type') === 'event' ? 'selected' : '' }}>Agenda / Seminar / Workshop</option>
                    </select>
                </div>
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                    ...
                    <input type="date" name="deadline" value="{{ old('deadline') }}" class="w-full px-3.5 py-2.5 rounded-xl border border-slate-200 text-xs focus:ring-2 focus:ring-red-500 outline-hidden bg-white" />
                </div>
                <div class="flex items-center gap-6 pt-6">
                    <label class="inline-flex items-center gap-2 cursor-pointer">
                        <input type="checkbox" name="is_pinned" value="1" {{ old('is_pinned') ? 'checked' : '' }} class="rounded border-slate-300 text-red-600 focus:ring-red-500 size-4" />
                        <span class="text-xs font-bold text-slate-700">Sematkan ke Atas (Pin to Top)</span>
                    </label>
                    <label class="inline-flex items-center gap-2 cursor-pointer">
                        <input type="checkbox" name="is_published" value="1" {{ old('is_published', '1') ? 'checked' : '' }} class="rounded border-slate-300 text-green-600 focus:ring-green-500 size-4" />
                        <span class="text-xs font-bold text-slate-700">Langsung Tayang (Publish)</span>
                    </label>
                </div>
            </div>
            ...
            <div>
                ...
                <textarea name="summary" rows="2" placeholder="Ringkasan 1-2 kalimat yang menarik untuk kartu pengumuman di portal..." class="w-full px-3.5 py-2.5 rounded-xl border border-slate-200 text-xs focus:ring-2 focus:ring-red-500 outline-hidden">{{ old('summary') }}</textarea>
            </div>
            ...
            <div>
                ...
                <textarea name="content" rows="8" required placeholder="Tuliskan detail pengumuman, ruang lingkup topik, panduan penulis, jadwal penting..." class="w-full px-3.5 py-2.5 rounded-xl border border-slate-200 text-xs focus:ring-2 focus:ring-red-500 outline-hidden leading-relaxed">{{ old('content') }}</textarea>
            </div>

            <!-- Banner & Attachment -->
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4 pt-2 border-t border-slate-100">
                <div>
                    <label class="block text-xs font-bold text-slate-700 uppercase mb-1">Banner Poster (Opsional, JPG/PNG max 3MB)</label>
                ...
                <div>
                    <label class="block text-xs font-bold text-slate-700 uppercase mb-1">Lampiran Berkas (Opsional, PDF/DOCX max 10MB)</label>
